...
$orderData = purchaseOrderByidForBasic($orderId);
$retailer_data = getRetailerById($orderData->retailer_id);
$vendorData = getVendor($orderData->vendor_id);
$company_detail = getCompanyDetailById($orderData->company_id);
$poDate = str_replace('/', '-', $orderData->po_date);
$poDate = date('d-M-Y', strtotime($poDate));


if (purchaseOrderByidCountForBasic($orderId) > 0) {
    ?>
    <html>
        <head>
            <style>
            ...
            <div class="container">
                    ...
                    <div class="left">
                        ...
                        <b> Supplier GSTIN No : <?php
                            if (!empty($orderData->vendor_id)) {
                                echo $vendor_gstin = getVendorGstinNoById($orderData->vendor_id);
                ...
                <div class="table-area">
                    <table class="table" border="1">
                        ...
                        <tbody>
                            <?php
                            $orderDetails = purchaseOrderDetailsForBasic($orderData->id);
                            $sr_NO = 1;
                            $rowspan = count($orderDetails);
                            foreach ($orderDetails as $orderDetail) {
                                $itemDetails = getItemDetails($orderDetail->item_id);
                                ?>
                                <tr>
                            ...
                            <tr>
                                ...
                                <td><b><?php echo amount($orderData->sub_total); ?></b></td>
                            </tr>
                            <tr>
