                                ...
                                <div class="page-header">
                                    <div class="widget-box">
                                        <div class="widget-header">
                                            <h4 class="widget-title">Retailer | Cash In Hand Details.</h4>
                                        </div>
...
//                                                    $sale_total = getRetailerSalesByDateCountAsOn($retailer_id, $f_start_date, $f_end_date);
                                                    $sale_total = getRetailerSalesByDateCountAsOnJoinDate($retailer_id, $f_start_date, $f_end_date);
                                                    $sale_discount_total = getRetailerSalesDiscountByDateCountAsOnJoinDate($retailer_id, $f_start_date, $f_end_date);
                                                    ...
                                                    $opening = $july_opening + $sale_total - $sale_discount_total - $approve_tra + $received_tra - $received_expense;

                                                    foreach ($date_range as $date) {
                                                        $for_date = $date->format('Y-m-d');
                                                        //Sales Credit
                                                        $sales_data = getRetailerSalesDetailsByDate($retailer_id, $for_date);
//                                                        $sales_data = getRetailerSalesByDateCountAsOnJoin($retailer_id, $for_date);
                                                        if (count($sales_data) > 0) {
                                                            foreach ($sales_data as $sale) {
                                                                $transaction_no = $sale->order_no;
                                                                $remarks = "" . $transaction_no;
                                                                $sale_discounts_data = getRetailerSalesDiscountDetailsByDate($retailer_id, $for_date, $transaction_no);
                                                                $credit = $sale->total_price - $sale_discounts_data->discount_amount;
                                                                $debit = 0;
                                                        ...
                                                        //Discount Sales
                                                        $sales_data_DIS = getRetailerSalesDiscountDetailsByDate($retailer_id, $for_date);
                                                        if (count($sales_data_DIS) > 0) {
                                                            foreach ($sales_data_DIS as $sale_discount) {
                                                                $transaction_no = $sale_discount->order_no;
                                                                $remarks = "" . $transaction_no;
                                                                $debit = $sale_discount->discount_amount;
                                                                $credit = 0;
                                                                ...
                                                                <tr>
                                                                    <td><?php echo $index; ?></td>
                                                                    <td><?php echo getRetailerNameById($retailer_id); ?></td>
                                                                    <td>SALE DISCOUNT - <?php echo $transaction_no; ?></td>
                                                                    <td><?php echo $for_date; ?></td>
                                                                    <td><?php echo IND_money_format($opening); ?></td>
                                                                    <td><?php echo IND_money_format($debit); ?></td>
                                                                    <td><?php echo IND_money_format($credit); ?></td>
