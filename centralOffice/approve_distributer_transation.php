                            ...
                            <div class="col-xs-12">
                                <div class="row">
                                    <div class="col-xs-12">
                                        ...
                                        <div class="row">
                                            <h3 class="header-text"> Transaction Approval Request.</h3>
                                            <div class="col-xs-12">
...
//                                                                if (isset($_POST['selection'])) {
                                                                    ...
                                                                    $getOrderRequest = getApproveTransactionRequestSelection($retailer_id, $date_1, $date_2, $selection, $company_id_in);
//                                                                } else {
...
//                                                                }
                                                                    ...
                                                                    if (count($getOrderRequest) > 0 && !empty($getOrderRequest)) {
                                                                        foreach ($getOrderRequest as $row) {
                                                                            ...
                                                                            echo "<tr class='tr_td_" . $row->id . "'>"
                                                                            . "<td>" . $i . "</td>";
                                                                            echo "<td>" . getRetailerNameById($row->retailer_id) . "</td>";
                                                                            if ($row->mode == 1) {
                                                                                echo "<td>Bank Deposit</td>";
                                                                            } else {
                                                                                echo "<td>Cash Transfer</td>";
                                                                            }
                                                                            echo '<td>';
                                                                            if ($row->mode == 1) {
                                                                                echo "" . getBankNameById($row->bank_id) . "";
                                                                            } else {
                                                                                echo "" . getRetailerNameById($row->bank_id) . "";
                                                                            }
                                                                            echo "</td><td>" . $row->amount . "</td><td>";
                                                                            echo date('d M Y', strtotime($row->transaction_date)) . "</td><td>";
                                                                            if ($row->transaction_no == 0) {
                                                                                echo "" . $row->transaction_remark . "</br>";
                                                                            } else {
                                                                                echo "" . $row->transaction_no . "</br>";
                                                                            }
                                                                            echo "</td>";
                                                                            if (empty($row->slip)) {
                                                                                echo '<td>NA</td>';
                                                                            } else {
                                                                                echo "<td><a target='blank' href='http://agrosindia.in/retailer/slip/" . $row->slip . "'>Download</a></td>";
                                                                            }
                                                                            echo "<td>" . date('d M Y H:i', strtotime($row->datetime)) . "</td>"
                                                                            . "<td>" . $statusmsg . "</td>"
                                                                            . "<td width='300'>";
																			
															if(date('Y-m-d', strtotime($row->transaction_date)) > $update_block_date){
                                                                            ?>
                                                                        <textarea class="remarks_<?php echo $row->id; ?>" name="remarks_<?php echo $row->id; ?>"><?php
                                                                            if (isset($row->remarks) && !empty($row->remarks)) {
                                                                                echo $row->remarks;
                                                                            }
                                                                            ?></textarea>
                                                                        <br/>
                ...
                function approveRequestTransaction(order_item_id)
                {
                    ...
                    if (confirm("Are you sure you want to approve this transaction?")) {
                        $.ajax({
                            type: 'POST',
                            url: 'ajax.php?menu=2',
                            data: {'types': 'approveTransaction', 'remarks': remarks, 'order_item_id': order_item_id, 'verify_data': verify_data},
                            success: function (result) {
                                result = $.trim(result);
                                if (result == 1) {
                                    alert('Transaction Saved Successfully.');
                                    $(".tr_td_" + order_item_id).css("display", "none");
//                                    window.location.href = "approve_distributer_transation.php?menu=30";
                                } else {
                                    alert('Something Wrong Try Again.');
//                                    window.location.href = "approve_distributer_transation.php?menu=30";
                                }
