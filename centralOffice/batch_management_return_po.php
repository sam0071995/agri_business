...
$limit = 100;
$all_orders = getOrderedBatchDetailsReturnPO($limit);

foreach ($all_orders as $all_orders) {
    $item_code = $all_orders->item_id;
    $retailer_id = $all_orders->retailer_id;
    $order_no = $all_orders->po_no;
    $batch_no = $all_orders->batch_no;
    $qty = $all_orders->qty;
    $add_date = date("Y-m-d", strtotime($all_orders->added_date));
    $m_id = $all_orders->unique_id;
    ...
    if ($blockedBatch == 0) {
        ...
        if ($availableBatch > 0) {
            if ($qty > 0) {
                ...
                $whereMaster = "unique_id='$m_id' AND batch_manage='0'";
                $updateMaster = update("purchase_order_return_detail", $dataUpdateMaster, $whereMaster);
                if ($updateMaster) {
                    $dataUpdate = array();
                    $dataUpdate['status'] = 7;
                    $dataUpdate['order_no'] = $order_no;
                    $dataUpdate['remarks'] = "BlockbyIT for ReturnPO";
