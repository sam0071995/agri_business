        ...
        <div class="main-container ace-save-state" id="main-container">
                            ...
                            <div class="col-xs-12">
                                <div class="row">
                                    <div class="col-xs-12">
                                        ...
                                        <div class="row">
                                            <h3 class="header-text"> Stock Transfer Approval Request.</h3>
                                            <div class="col-xs-12">
                                                <div class="row">
                                                        ...
                                                        <table id="dynamic-table" class="table table-bordered table-hover">
                                                            <thead class="thead-dark">
                                                                <tr>
                                                                    ...
                                                                    <th width="15%" align="left">Dispatch No</th>
                                                                    <th width="15%" align="left">From Retailer</th>
                                                                    <th width="15%" align="left">To Retailer</th>
                                                            ...
                                                            <tbody>
                                                                ...
                                                                $status = 0;
                                                                $i = 1;
                                                                $getOrderRequest = getApproveStockRequestIN($company_id_in);
                                                                if (count($getOrderRequest) > 0 && !empty($getOrderRequest)) {
                                                                    foreach ($getOrderRequest as $row) {
                                                                        echo "<tr>"
                                                                        . "<td>" . $i . "</td>"
                                                                        . "<td>" . $row->order_no . "</td>"
                                                                        . "<td>" . getRetailerNameById($row->frm_retailer_id) . " <b>(stock : " . getRetailerItemCurrentStockById($row->item_code, $row->frm_retailer_id) . ")</b></td>"
                                                                        . "<td>" . getRetailerNameById($row->retailer_id) . " <b>(stock : " . getRetailerItemCurrentStockById($row->item_code, $row->retailer_id) . ")</b></td>"
                                                                        . "<td>" . getItemNameByItemCode($row->item_code) . "</td>"
                                                                        . "<td>" . $row->batch_no . "</td>"
                                                                        . "<td>" . $row->expire_date . "</td>"
                                                                        . "<td>" . $row->req_qty . "</td>"
                                                                        . "<td>" . date('d-M-Y', strtotime($row->add_date)) . "<br />" . $row->add_datetime . "</td>"
                                                                        . "<td width='300'>";
                                                                        ?>
                                                                    <input type="text" name="challan_no" placeholder="Enter Challan Number" class="challan_no_<?php echo $row->id; ?>" />
                                                                    <br/><br/><button type='button' onclick="approveRequestOrder(<?php echo $row->id; ?>);" class='button btn-success' style='cursor:pointer' title='Inward'>Approve</button>&nbsp;&nbsp;&nbsp;
                                                                    <button type='button' onclick="rejectRequestOrder(<?php echo $row->id; ?>);" class='button btn-danger' style='cursor:pointer' title='Reject'>Reject</button>
                                                                    <?php
                                                                    echo "</td>
							</tr>";
                                                                    $i++;
            ...
            <script type="text/javascript">
                ...
                function released_purchase_click() {
                    window.location.href = "release_purchase_order.php?menu=21";
                }
                ...
                function release_purchase(purchase_id) {
                    if (confirm("Are you sure you want to Release this?")) {
                        $.ajax({
                            type: 'POST',
                            url: 'release_purchase_order.php?menu=21',
                            data: {'purchase_id': purchase_id},
                            success: function (result) {
                                result = $.trim(result);
                                if (result) {
                                    window.location.href = "release_purchase_order.php?menu=21&success=realese";
                                } else {
                                    alert('Something Wrong Try Again.');
                                }
                            }
                ...
                function approveRequestOrder(order_item_id)
                {
                    if (confirm("Are you sure you want to approve this item?")) {
                        var challan_no = $(".challan_no_" + order_item_id).val();
                        $.ajax({
                            type: 'POST',
                            url: 'ajax.php?menu=21',
                            data: {'types': 'approveRequest', 'order_item_id': order_item_id, 'challan_no': challan_no},
                            success: function (result) {
                                result = $.trim(result);
                                if (result == 1) {
                ...
                function rejectRequestOrder(order_item_id)
                {
                    if (confirm("Are you sure you want to reject this item?")) {
                        $.ajax({
                            type: 'POST',
                            url: 'ajax.php?menu=21',
                            data: {'types': 'rejectRequest', 'order_item_id': order_item_id},
                            success: function (result) {
                                result = $.trim(result);
                                if (result == 1) {
                                    alert('Stock Reject Successfully.');
                                    window.location.href = "approve_stock_transfer.php?menu=21";
                                } else {
                                    alert('Something Wrong Try Again.');
                                    window.location.href = "approve_stock_transfer.php?menu=21";
                ...
                function copy_purchase(purchase_id)
                {
                    if (confirm("Are you sure you want to copy this?")) {
                        $.ajax({
                            type: 'POST',
                            url: 'copy_purchase_order.php?menu=21',
                            data: {'purchase_id': purchase_id},
                            success: function (result) {
                                result = $.trim(result);
                                if (result == 1) {
                                    window.location.href = "purchase_order.php?menu=21&success=copy";
                                } else {
                                    alert('Something Wrong Try Again.');
                                    window.location.href = "purchase_order.php?menu=21&failure=1";
                                }
                            }
                        });
                    }
                ...
                function delete_purchase(purchase_id)
                {
                    if (confirm("Are you sure you want to Delete this?")) {
                        $.ajax({
                            type: 'POST',
                            url: 'delete_purchase_order.php?menu=21',
                            data: {'purchase_id': purchase_id},
                            success: function (result) {
                                result = $.trim(result);
                                if (result == 1) {
                                    window.location.href = "purchase_order.php?menu=21&success=delete";
                                } else {
                                    alert('Something Wrong Try Again.');
                                    window.location.href = "purchase_order.php?menu=21&failure=1";
                                }
                ...
                function show_detail()
                {
                    ...
                    if (vendor_id_ajit != '') {
                        var xmlhttp;
                        var url = "ajax_agro.php??menu=21&type=vendor&id=" + vendor_id_ajit;
                ...
                function item_unit()
                {
                    ...
                    var xmlhttp;
                    var url = "ajax_agro.php??menu=21&type=itemunit&id=" + res[0];
