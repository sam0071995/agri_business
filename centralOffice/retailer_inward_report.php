                                                        ...
                                                        <table id="dynamic-table" class="table table-bordered table-hover">
                                                            <thead class="thead-dark">
                                                                <tr>
                                                                    ...
                                                                    <th width="15%" align="left">Supplier GSTIN</th>
                                                                    ...
                                                                    <th width="15%" align="left">ItemUnit</th>
                                                                    ...
                                                                    <th width="15%" align="left">Taxable Value</th>
                                                                    <th width="15%" align="left">CGST Value</th>
                                                                    <th width="15%" align="left">SGST Value</th>
                                                                    <th width="15%" align="left">IGST Value</th>
                                                                    <th width="15%" align="left">Total Value</th>
                                                                    <th width="15%" align="left">PO Discount</th>
...
//                                                                    $purchaseOrder = getInwardDataByRetailerIdFilterJoinReport($date_1, $date_2, $retailer_id, $company_id_in, $status_filter, $item_code, $status_report);
                                                                    $purchaseOrder = getInwardDataByRetailerIdFilterJoinReportInventoryGRN($date_1, $date_2, $retailer_id, $company_id_in, $status_filter, $item_code, $status_report);
                                                                    foreach ($purchaseOrder as $row) {
                                                                        $transfered_qty = 0;
                                                                        $purchase_qty = 0;
                                                                        $GSTIN_NO_VENDOR = "NA";
                                                                        $GSTIN_NO_company = getCompanyGSTINById($row->company_id);
                                                                        if ($row->dispatch_retailer_id == 0) {
                                                                            $GSTIN_NO_VENDOR = getVendorGstinNoById($row->vendor_id);
                                                                            if (empty($GSTIN_NO_VENDOR)) {
                                                                                $GSTIN_NO_VENDOR = "NA";
                                                                            }
                                                                            if ($row->po_type == 1) {
                                                                                $purchase_type = 'Purchase Order';
                                                                            } else if ($row->po_type == 2) {
                                                                                $purchase_type = 'Credit Note';
                                                                            } else {
                                                                                $purchase_type = 'Purchase Order';
                                                                            }
                                                                        } else {
                                                                            $purchase_type = 'Transfered';
                                                                        }
                                                                        if ($row->dispatch_retailer_id == 0) {
                                                                        ...
                                                                        if ($row->dispatch_retailer_id == 0) {
                                                                            $firstTwo_GSTIN_NO_VENDOR = substr($GSTIN_NO_VENDOR, 0, 2);
                                                                        } else {
                                                                            $firstTwo_GSTIN_NO_VENDOR = substr($GSTIN_NO_company, 0, 2);
                                                                        }
                                                                        $firstTwo_GSTIN_NO_company = substr($GSTIN_NO_company, 0, 2);

                                                                        if ($row->dispatch_retailer_id == 0) {
                                                                            $total_taxable_value = amount($row->po_basic * $purchase_qty);
                                                                        } else {
                                                                            $total_taxable_value = amount($row->po_basic * $transfered_qty);
                                                                        }
                                                                        if ($row->po_type == 1) {
                                                                            if ($temp_purchase_order == $row->po_no) {
                                                                                $po_discount = 0;
                                                                            } else {
                                                                                $po_discount = amount($row->po_discount);
                                                                            }
                                                                        } else {
                                                                            $po_discount = 0;
                                                                        }
...
//                                                                        }
                                                                        ...
                                                                        <tr>
                                                                            ...
                                                                            <td><?php echo $GSTIN_NO_VENDOR; ?></td>
                                                                            ...
                                                                            <td><?php echo getItemUNITByItemCode($row->item_desc); ?></td>
                                                                            ...
                                                                            <td><?php echo amount($total_taxable_value); ?></td>
                                                                            <?php if ($firstTwo_GSTIN_NO_VENDOR == $firstTwo_GSTIN_NO_company) { ?>
                                                                                <td><?php echo amount(($total_taxable_value * ($row->po_gst / 2)) / 100); ?></td>
                                                                                <td><?php echo amount(($total_taxable_value * ($row->po_gst / 2)) / 100); ?></td>
                                                                                <td><?php echo amount(0); ?></td>
                                                                            <?php } else { ?>
                                                                                <td><?php echo amount(0); ?></td>
                                                                                <td><?php echo amount(0); ?></td>
                                                                                <td><?php echo amount(($total_taxable_value * ($row->po_gst)) / 100); ?></td>
                                                                            <?php } ?>
                                                                            <td><?php echo amount((($total_taxable_value * ($row->po_gst)) / 100) + $total_taxable_value); ?></td>
