...
function getRetailerTransferPurchareonDateMailBetween($retail_id, $item_code, $date_1, $date_2) {
    $tbl_fields = "sum(total_basic) as total_basic,count(pd.id) as count,SUM(pd.`qty`) AS qty,sum(pd.amount) as amount,sum(pd.rate) as rate,sum(pd.gst_rate) as gst_rate,pd.retailer_id,p.vendor_id as vendor_id";
    $table_name = "`purchase_order_return` p,`purchase_order_return_detail` pd";
...
function update($table_name = '', $data = '', $where = '') {
    ...
    $query = "UPDATE $table_name SET $field_q $where";
    if ($table_name == 'purchase_order_basic') {
//        echo $query;
//        exit;
    }
    if (mysqli_query($conn, $query)) {
